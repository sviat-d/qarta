...
/**
 * Get a Stripe client that acts on behalf of a connected account.
 * Uses the platform secret key with the Stripe-Account header.
 */
export function getStripeClientForAccount(stripeAccountId: string): Stripe {
  if (!config.STRIPE_SECRET_KEY) {
    throw new Error("STRIPE_SECRET_KEY is not configured");
  }
 ...
 * Uses the platform key with stripeAccount header for connected accounts.
 */
